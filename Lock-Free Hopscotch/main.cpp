...
void worker(HopscotchHash *h, int tid)
{
    ...
    {
        h->contains(dataset[randArray[tid * contains + i]]);
    ...
    {
         key = randArray[tid * deletes + x];//dataset[index];
         h->Remove(key);
    }
...
int main (int argc, char* argv[])
{
  ...
  printf("build time  %lf  MQPS \n",  (float)TOTDATA/((stop.tv_sec * 1000000 + stop.tv_usec)
